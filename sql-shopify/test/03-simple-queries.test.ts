...
describe("Simple Queries", () => {
  ...
  it(
    "should select app count with rating of 5 stars",
    async (done) => {
      const query = `SELECT COUNT(1) as count FROM ${APPS}
        WHERE rating = 5`;
  ...
  it(
    "should select top 3 develepors with most apps published",
    async (done) => {
      const query = `SELECT COUNT(1) AS count, developer FROM ${APPS}
        GROUP BY developer
        ORDER BY count DESC
        LIMIT 3`;
  ...
  it(
    ...
    async (done) => {
      const query = `SELECT substr(date_created, 7, 4) as year ,COUNT(1) AS review_count FROM ${REVIEWS}
