...
describe("Insert Data", () => {
  ...
  it(
    ...
    async (done) => {
      ...
      const rowByTitle = await db.selectSingleRow(
        selectCategoryByTitle("Places to sell")
      );
  ...
  it(
    "should insert app categories data",
    async (done) => {
      ...
      const appCategoriesRows = await db.selectMultipleRows(
        selectAppCategoriesByAppId(1056)
      );
      expect(appCategoriesRows).toEqual([
        { app_title: "mmuze", category_id: 1, category_title: "Store design" },
        {
          app_title: "mmuze",
          category_id: 2,
          category_title: "Sales and conversion optimization",
        },
        { app_title: "mmuze", category_id: 3, category_title: "Marketing" },
        {
          app_title: "mmuze",
          category_id: 5,
          category_title: "Customer support",
        },
      ]);
      done();
    },
  ...
  it(
    "should insert key benefits data",
    async (done) => {
      ...
      const uniqueValues = await db.selectSingleRow(
        selectUnigueRowCount(KEY_BENEFITS, "description")
      );
  ...
  it(
    "should insert pricing plans",
    async (done) => {
      const pricingPlans = await ShopifyCsvLoader.loadUniquePricingPlans(
        pricingPlansDir
      );
  ...
  it(
    ...
    async (done) => {
      ...
      const uniqueAuthors = await db.selectSingleRow(
        selectUnigueRowCount(REVIEWS, "author")
      );
  ...
  it(
    "should insert apps pricing plans data",
    async (done) => {
